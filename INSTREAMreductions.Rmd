...
Here we would use median values because there are outliers and apparent skewness from the boxplot. The mean reduction is even greater (%72). Negative value represents a dcrease relative to the historical data.
...
Here we would use median values because there are outliers and apparent skewness from the boxplot. Postive values represents a relative increase. This relative change can range from (-) 100% decrease to (+) infinite increase.
...
$H_a:$ our recent samples had less total suspended solids than our historical observations
...
wilcox.test(y= oldporter$TSS, x= instreamdata[instreamdata$LOCCODE == "PB", "TSS"],
            alternative = "greater")
```
$H_a:$ new data is greater than the old data
...
Ha: new observations < old observations.
...
```
 ...
 Negative value here represents a twenty percent reduction over the historical observations.
...
Reject null hypothesis that porter bayou TN concentrations are $\leq$ old observations. <b> $H_a:$ new observations are greater than old ones.
...
```
 ...
 This postive value represents an ncrease from the historical observations.
